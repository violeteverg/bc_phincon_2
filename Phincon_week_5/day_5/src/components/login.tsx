...
import { fetchCookieRequest } from "../utils/features/login/authSlice";

const Login = () => {
  const dispatch = useDispatch();

  const { cookie } = useSelector((state: any) => state.cookie);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    console.log(dispatch(fetchCookieRequest()), "test");
    ...
    if (code !== 200) {
      ...
      json.details?.map((detail: { message: string }) => {
